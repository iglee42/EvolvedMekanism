...
public class EMItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        ...
        for (Fluid fluid : BuiltInRegistries.FLUID) {
            ...
            withExistingParent(id.getPath() + "_bucket", ResourceLocation.fromNamespaceAndPath("neoforge", "item/bucket"))
    ...
    private void handheldTool(String material) {
        for (String tool : TOOLS) {
            ...
            if (BuiltInRegistries.ITEM.get(EvolvedMekanism.rl(path)) == Items.AIR) {
                continue;
            }
    ...
    private void shield(String material) {
        ...
        getBuilder(path)
                ...
                .texture("particle", particle)
                .override()
                .predicate(ResourceLocation.fromNamespaceAndPath("mekanismtools", "blocking"), 1)
                .model(blocking)
                .end();
    }

    private void energyCube(String path) {
        withExistingParent(path, ResourceLocation.fromNamespaceAndPath("mekanism", "item/basic_energy_cube"));
    }

    private void fluidTank(String path) {
        withExistingParent(path, ResourceLocation.fromNamespaceAndPath("mekanism", "item/basic_fluid_tank"));
    }
}
